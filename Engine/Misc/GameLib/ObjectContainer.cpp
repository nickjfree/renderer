...
CObjectContainer::CObjectContainer(void):m_FirstPool(0),m_FreeItem(0),m_ObjectSize(0),m_ItemSize(0),m_MemUsed(0),m_PoolNum(0),m_UsedItemNum(0),m_UsedItem(0)
{
}

CObjectContainer::CObjectContainer(int ObjectSize) : m_FirstPool(0), m_FreeItem(0), m_ObjectSize(0), m_ItemSize(0), m_MemUsed(0), m_PoolNum(0), m_UsedItemNum(0), m_UsedItem(0)
{
	Init(ObjectSize);
}
...
int CObjectContainer::Init(int ObjectSize)
{
...
int CObjectContainer::MakeFreePool(PoolHeader * Pool)
{
...
int CObjectContainer::ToFreeItem(ItemHeader * Header)
{
...
int CObjectContainer::AllocOne(void ** pData)
{
	int id = -1;
	if(!m_FreeItem)
	{
...
int CObjectContainer::ReleaseItem(ItemHeader * Header)
{
	ToFreeItem(Header);
	return 0;
}

void * CObjectContainer::GetObjectByID(int Id)
{
	ItemHeader * Header;
	Header = GetItem(Id);
	return Header->data;
...
int CObjectContainer::ReleaseOne(int Id)
{
...
ItemHeader * CObjectContainer::GetItem(int id)
{
	int FindID = id;
	if(!m_FirstPool)
		return NULL;
...
int CObjectContainer::Destroy(void)
{
	//Free all pools
	return 0;
}

int CObjectContainer::ToUsedItem(ItemHeader * Header)
{
	Header->next_item = NULL;
	if(m_UsedItem)
	{
...
void * CObjectContainer::GetNext(void * Object)
{
	ItemHeader * Header;
	if(!m_UsedItemNum)
	{
