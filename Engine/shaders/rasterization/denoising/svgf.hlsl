...
// float2 GetPrevScreenCoord(float2 uv, out float valid, out float fwidthZ)
// {
...
{
    GBufferContext gbuffer = GetGBufferContext(uv);
    return gbuffer.ViewSpaceNormal;
}
...
*/
PS_Output PS_TemporalAccumulation(PS_Input ps_input)
{   
    ...
    float valid = gbuffer.ReprojecttionValid;
    float fwidthZ = gbuffer.FwidthZ;
    float2 prevUV = gbuffer.PrevUV;
...
*/
PS_Output PS_FilterVariant(PS_Input ps_input)
{
    PS_Output output = (PS_Output)0;
    // uv
    float2 uv = ps_input.TexCoord;
    ...
    float4 moments = gMoment.SampleLevel(gSamPoint, uv, 0);
    float history = moments.w;
    float  centerLinearZ = GetLinearZ(uv);
    // get texel size
    float2 demension;
...
*/
PS_Output PS_Filter(PS_Input ps_input)
{   
    PS_Output output = (PS_Output)0;
